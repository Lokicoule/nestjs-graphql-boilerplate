  ...
  public generateCode(criteria: SettingCriteria): Observable<string> {
    return this.settingsService.findSetting(new SettingCriteria(criteria)).pipe(
      map((setting) => {
        setting =
          setting ||
          ProductSettingsUseCase.initializeCodeGeneratorSetting(
            criteria.authorId,
          );
        return this.settingsService.createOrUpdateSetting(
          new SettingCriteria(criteria),
          ProductSettingsUseCase.incrementCounter(setting),
        );
      }),
      switchMap((setting$) =>
        setting$.pipe(
          map((setting) => ProductSettingsUseCase.buildCode(setting)),
        ),
      ),
    );
  }
